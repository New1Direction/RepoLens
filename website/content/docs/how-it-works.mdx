...
- **OpenRouter Free router** — after OpenRouter OAuth, choose **OpenRouter Free router**. Live free variants are marked **★ Free** and stay at the top of the picker; they use OpenRouter’s `:free` model variant. Availability and rate limits vary, so connect Gemini or Ollama as a fallback instead of relying on it alone.
